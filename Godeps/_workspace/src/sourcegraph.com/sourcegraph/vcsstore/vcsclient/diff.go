...
func (r *repository) CrossRepoDiff(base vcs.CommitID, headRepo vcs.Repository, head vcs.CommitID, opt *vcs.DiffOptions) (*vcs.Diff, error) {
	...
	url, err := r.url(RouteRepoCrossRepoDiff, map[string]string{"Base": string(base), "HeadVCS": headRepo2.vcsType, "HeadCloneURL": headRepo2.cloneURL.String(), "Head": string(head)}, opt)
	if err != nil {
		return nil, err
	}
